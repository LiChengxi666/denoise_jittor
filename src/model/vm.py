...
def patch_based_denoise(model: VelocityModule, pcl_noisy, patch_size=1000, seed_k=6, seed_k_alpha=1) -> jt.Var:
    ...
    # Precompute coverage: which points appear in at least one patch
    point_idxs_np = point_idxs.numpy().flatten()  # (P*M,)
    covered_set = set(point_idxs_np.tolist())

    pcl_out = []
    for pidx in range(N):
        if pidx not in covered_set:
