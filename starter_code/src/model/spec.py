...
class ModelSpec(nn.Module, ABC):
    ...
    def __init__(self, model_config, transform_config):
        ...
        if not isinstance(model_config, dict):
            assert OmegaConf is not None, "OmegaConf is required for non-dict model_config"
            model_cfg = OmegaConf.to_container(model_config, resolve=True)
        else:
            model_cfg = model_config
        if not isinstance(transform_config, dict):
            assert OmegaConf is not None, "OmegaConf is required for non-dict transform_config"
