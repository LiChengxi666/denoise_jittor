    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            if validate_dataloader is not None:
                ...
                if isinstance(validate_dataloader, dict):
                    ...
                    for batch in pbar:
                        ...
                        self.on_validation_batch_end()
                mean_val_loss = self.on_validation_epoch_end()
            else:
                mean_val_loss = None
            
            if self.save_last:
                self._save_checkpoint(self._checkpoint_path(f'{self.ckpt_save_name}_last'), 'last')
            if self.save_best and mean_val_loss is not None and mean_val_loss < self.best_validation_loss:
                ...
                self._save_checkpoint(self._checkpoint_path(f'{self.ckpt_save_name}_best'), f'best val {mean_val_loss:.8f}')
